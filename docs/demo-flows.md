...
5. Tap the row matching advertised name **`RN_BLE_HR_Demo`** (or filtered device).
6. Observe **HR** and **Battery** lines updating; logs show notify payloads.

## Flow B — Nordic LBS profile

1. Peripheral: select **Nordic LED Button Service**, **Start peripheral**.
2. Central: target **Nordic LBS**, scan, connect to **`My_LBS`** (or matching device).
3. Peripheral UI: use **Button** toggle — central logs should show button notify values.
4. Central: tap **LED ON** / **LED OFF** — peripheral **LED** readonly field reflects `ledState` from writes (when state machine is **active**). In **idle**, writes are log-only per profile.
